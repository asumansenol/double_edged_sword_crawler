// Source: https://searchfox.org/mozilla-central/source/toolkit/components/satchel/SignUpFormRuleset.sys.mjs
...
function createRuleset(coeffs, biases) {
  ...
  function docTitleMatchesRegisterRegex(fnode) {
    ...
    return checkValueAgainstRegex(docTitle, registerRegexFathom);
  }
  ...
  function closestHeaderMatchesLoginRegex(fnode) {
    return closestHeaderMatchesPredicate(fnode.element, (header) =>
      checkValueAgainstRegex(header.innerText, loginRegexFathom)
    );
  }
  function closestHeaderMatchesRegisterRegex(fnode) {
    return closestHeaderMatchesPredicate(fnode.element, (header) =>
      checkValueAgainstRegex(header.innerText, registerRegexFathom)
    );
  }
  ...
  function formHasRegisterButton(fnode) {
    return elementHasPredicateMatchingButton(
      fnode.element,
      (button) =>
        checkValueAgainstRegex(button.innerText, registerRegexFathom) ||
        buttonMatchesPredicate(button, (attr) =>
          checkValueAgainstRegex(attr, registerRegexFathom)
        )
    );
  }
  function formHasLoginButton(fnode) {
    return elementHasPredicateMatchingButton(
      fnode.element,
      (button) =>
        checkValueAgainstRegex(button.innerText, loginRegexFathom) ||
        buttonMatchesPredicate(button, (attr) =>
          checkValueAgainstRegex(attr, loginRegexFathom)
        )
    );
  }
  ...
  function formAttributesMatchLoginAndRegisterRegex(fnode) {
    return formMatchesPredicate(fnode.element, (attr) =>
      checkValueAgainstAllRegex(attr, [registerRegexFathom, loginRegexFathom])
    );
  }
  ...
  const rules = ruleset(
    [
      rule(
        DEVELOPMENT ? dom("form").when(checkElementVisibilityForFathom) : element("form"),
...
const signupTrainees = new Map();

const signupTrainee = {
  coeffs: coefficients.form,
  viewportSize: { width: 1024, height: 768 },
  rulesetMaker: () => createRuleset(coefficients.form, biases.form),
};

signupTrainees.set("form", signupTrainee);


function hasSignupForm() {
    let detectedForm = false;
    try {
      const forms = signupTrainees.get('form').rulesetMaker().against(document).get('form');
      for (const form of forms) {
          const scoreForForm = form.scoreFor('form');
          if (scoreForForm > .5) {
              detectedForm = true;
              break;
          }
